...
def _get_follow_obj(*, user: User, follower: User) -> Follow:
    return Follow.objects.get(user=user, follower=follower)
...
def _validate_follow_request(*, user: User, follower: User) -> None:
    ...
    if Follow.objects.filter(user=user, follower=follower, deleted_datetime__isnull=True).exists():
...
def _validate_unfollow_request(*, user: User, follower: User) -> None:
    ...
    if not Follow.objects.filter(user=user, follower=follower, deleted_datetime__isnull=True).exists():
